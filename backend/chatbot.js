/* backend/chatbot.js
   Unified chatbot logic:
    - loads JSON datasets from backend/data/*.json
    - builds Fuse indices when Fuse is available
    - lemmatizes via compromise (nlp) when available (fallback to simple tokenization)
    - optional spell correction hook (if `spellCorrector` exists globally)
    - uses dayjs for relative day handling if available (fallback to Date)
    - exposes getResponse(rawInput) (and processInput alias) for UI pages
    - sets window.__museumLoaded when load completes
*/
...
let artworksData = []; // if you have a separate artworks file
let __fusesBuilt = false;

window.__museumLoaded = false;

// Fuse indices (if Fuse available)
...
/* -------------------------
   Load JSON datasets
   ------------------------- */
// paths relative to this script's runtime location - adjust if needed
function loadJSONData() {
    const paths = [
        ...
        'backend/data/artworks.json' // optional - may 404, handled below
    ];
    ...
    Promise.all(fetches)
        ...
        .catch(err => {
            ...
            // still attempt to build fuses (will use empty arrays)
            try { buildFuses(); } catch(e){/*ignore*/ }
            window.__museumLoaded = true;
        });
}
...
/* -------------------------
   Build Fuse indices (if available)
   ------------------------- */
function buildFuses() {
    ...
    try {
        ...
        // Art by title & artist (if dataset available)
        ...
        // Map categories and gallery names
        const categories = [];
...
// sanitize: lower + remove weird punctuation (but keep apostrophes) + normalize whitespace
...
// lemmatizeTokens using compromise if available, fallback to simple tokens
...
// spelling correction hook (no-op if not available)
...
/* -------------------------
   Handlers (museum, exhibitions, art, map, generic)
   ------------------------- */

// Generic smalltalk / fallback
...
// Museum info: name, location, phone, hours, parking
...
function handleExhibitions(raw, s, lemmas) {
    ...
    // fuzzy match exhibition by name
...
// Map / locations handler
function handleMapLocations(raw, s, lemmas) {
    // match by category via fuseMapCategories
    ...
    // explicit gallery number lookup
    ...
    // coat-check, elevators, stairs similar...
...
// expose to browser globals and CommonJS (if used)
